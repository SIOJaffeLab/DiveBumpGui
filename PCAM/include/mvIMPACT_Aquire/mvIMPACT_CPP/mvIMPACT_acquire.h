...
//-----------------------------------------------------------------------------
{
    ...
    explicit BufferPart( HOBJ hObj ): ComponentCollection( hObj ), m_imageBufferDesc( 1 ), address(), dataSize(), dataType(),
    ...
    //-----------------------------------------------------------------------------
    {
        if( m_pRefData->m_bufferPartIterator.isValid() )
        {
            ...
            while( it.isValid() )
            {
                ...
                else if( m_pRefData->m_bufferParts[bufferPartCount]->hObj() != it.hObj() )
                {
                    // update existing ones if necessary
                    m_pRefData->m_bufferParts[bufferPartCount]->bindPublicProperties( it.hObj() );
                }
                ++bufferPartCount;
                ++it;
            }
