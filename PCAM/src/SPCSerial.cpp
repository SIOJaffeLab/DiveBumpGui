...
char SPCSerial::Start(string foldername)
{
    signed int errno;
    ...
    pthread_t thread;
    errno = pthread_create(&thread, 0, SPCSerial::entryPoint, this);
    if (errno)
    {
         stringstream ss;
...
//Allows the SPCSerial to enter a thread within its own class
void *SPCSerial::entryPoint(void* pthis)
{
    reinterpret_cast<SPCSerial *>(pthis)->threadedFunction();
    return pthis;
}

//SPCSerial thread: grabs all the bytes available on a device's buffer and writes them to a data file
...
char SPCSerial::threadedFunction()
{
    quit = false;
    int bAvail;
    bNew = false;
...
int SPCSerial::parseData(string s, char firstchar, char lastchar)
{
...
char SPCSerial::timeInsert(string s)
{
    int length;
    int pos;
