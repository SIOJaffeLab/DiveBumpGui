...
char PA200::Start(string foldername)
{
    signed int errno;
    ...
    pthread_t thread;
    errno = pthread_create(&thread, 0, PA200::entryPoint, this);
    if (errno)
    {
         stringstream ss;
...
//Allows the PA200 to enter a thread within its own class
void *PA200::entryPoint(void* pthis)
{
    reinterpret_cast<PA200 *>(pthis)->threadedFunction();
    return pthis;
}

//PA200 thread: grabs all the bytes available on a device's buffer and writes them to a data file
...
int PA200::parseData(string s, char firstchar, char lastchar)
{
...
char PA200::timeInsert(string s)
{
    int length;
    int pos;
